...
const main = async () => {
    ...
    const spinner = ora("Install new oracle proxy code...").start();

    const acc_artifacts = await locklift.factory.getContractArtifacts("OracleProxy");
    await locklift.tracing.trace(
        vault.methods
            .updateOracleProxyCode({
    ...
    spinner.succeed(`New oracle proxy code installed`);
};
